...
//public class CurrencyImporterIntegrationTests(ITestOutputHelper testOutputHelper)
//{
//    [Fact]
//    public async Task GetAllCountriesAsync_ReturnsNonEmptyCountryList()
//    {
//        var currencyImporter = new CurrencyImporter();
//        var iso4217Importer = new Iso4217Importer();

//        var currenciesList = await currencyImporter.Import();
//        var iso4217 = await iso4217Importer.Import();

//        Assert.NotNull(currenciesList);
//        Assert.NotNull(iso4217);

//        var empty = iso4217.CurrencyTable.CurrencyCountries.Where(x => string.IsNullOrEmpty(x.CurrencyNumber)).ToList();
//        var valid = iso4217.CurrencyTable.CurrencyCountries.Where(x => string.IsNullOrEmpty(x.CurrencyNumber) == false).ToList();

//        var isoListByKey = valid.GroupBy(x => x.Currency).ToDictionary(x => x.Key, x => x.First().CurrencyNumber);

//        var currenciesListByKey = currenciesList.Data;

//        var commonKey = currenciesListByKey.Keys.Intersect(isoListByKey.Keys).ToList();
//        var moreIso = isoListByKey.Keys.Except(commonKey).ToList();
//        var moreCurrencies = currenciesListByKey.Keys.Except(commonKey).ToList();

//        if (moreIso.Any())
//        {
//            testOutputHelper.WriteLine("More ISO: " + string.Join(", ", moreIso));
//        }

//        if (moreCurrencies.Any())
//        {
//            testOutputHelper.WriteLine("More moreCurrencies: " + string.Join(", ", moreCurrencies));
//        }

//        foreach (var key in commonKey)
//        {
//            var currencyApi = currenciesListByKey[key];
//            var iso = isoListByKey[key];

//			if(int.TryParse(iso, out var numericCode))
//				continue;

//            var currency = new Currency(key, numericCode, currencyApi.Name, currencyApi.Symbol, currencyApi.Decimal_digits, currencyApi.Rounding);

//            testOutputHelper.WriteLine(@$"        public static readonly Currency {currency.AlphabeticCode} = new(""{currency.AlphabeticCode}"", {currency.NumericCode}, ""{currency.Name}"", ""{currency.Symbol}"", {currency.DecimalDigits}, {currency.Rounding});");
//        }
//    }

//    [Fact]
//    public void DeserializeFromCurrencyApi()
//    {
//        var json = @"
//{
//    ""data"": {
...
//        // Act
//        var result = JsonConvert.DeserializeObject<CurrencyDataContainer>(json);

//        // Assert
//        Assert.NotNull(result);
//        Assert.NotEmpty(result.Data);

//		var currencyAED = result.Data["AED"];

//		Assert.Equal("AED", currencyAED.Code);
//        Assert.Equal(2, currencyAED.Decimal_digits);
//        Assert.Equal("United Arab Emirates Dirham", currencyAED.Name);
//        Assert.Equal("UAE dirhams", currencyAED.Name_plural);
//        Assert.Equal(0, currencyAED.Rounding);
//        Assert.Equal("AED", currencyAED.Symbol);
//        Assert.Equal("د.إ", currencyAED.Symbol_native);
//        Assert.Equal("fiat", currencyAED.Type);
//    }

//    // ...

//    [Fact]
//    public void Deserialize_ISO_4217()
//    {
//        var xml = @"		
...
//        using (TextReader reader = new StringReader(XmlToTest))
//        {
//            var iso4217 = (Iso4217)serializer.Deserialize(reader);
...
//    private static readonly string XmlToTest = 
